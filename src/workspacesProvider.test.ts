...
class TestableWorkspaceProvider extends WorkspaceProvider {
	...
	public updateAgentWatchers(
		workspaces: Workspace[],
		restClient: MockRestClient,
	) {
		return super.updateAgentWatchers(workspaces, restClient as never);
	}

	public createAgentWatcher(agentId: string, restClient: MockRestClient) {
		return super.createAgentWatcher(agentId, restClient as never);
	}

	public createWorkspaceTreeItem(workspace: Workspace) {
		return super.createWorkspaceTreeItem(workspace);
	}

	public getWorkspaceChildren(element: WorkspaceTreeItem) {
		return super.getWorkspaceChildren(element);
	}

	public getAgentChildren(element: vscode.TreeItem) {
		return super.getAgentChildren(element);
	}

	// Allow access to private properties for testing using helper methods
	public getWorkspaces(): WorkspaceTreeItem[] | undefined {
		return (this as never)["workspaces"];
	}

	public setWorkspaces(value: WorkspaceTreeItem[] | undefined) {
		(this as never)["workspaces"] = value;
	}

	public getFetching(): boolean {
		return (this as never)["fetching"];
	}

	public setFetching(value: boolean) {
		(this as never)["fetching"] = value;
	}

	public getVisible(): boolean {
		return (this as never)["visible"];
	}

	public setVisible(value: boolean) {
		(this as never)["visible"] = value;
	}
}
...
describe("WorkspaceProvider", () => {
	...
	describe("fetchAndRefresh", () => {
		...
		it("should not fetch when already fetching", async () => {
			...
			const handleVisibilitySpy = vi
				.spyOn(provider, "handleVisibilityChange")
				.mockImplementation(() => {});
		...
		it("should handle fetch errors gracefully", async () => {
			mockRestClient.getWorkspaces.mockRejectedValue(
	...
	describe("edge cases", () => {
		...
		it("should handle workspace query for All workspaces", async () => {
			...
			// Mock the handleVisibilityChange to prevent automatic fetchAndRefresh
			const handleVisibilitySpy = vi
				.spyOn(allProvider, "handleVisibilityChange")
				.mockImplementation(() => {});
...
describe("WorkspaceTreeItem", () => {
	...
	it("should set context value based on agent count", async () => {
		...
		// Test single agent
		const singleAgent: WorkspaceAgent = {
			id: "agent-1",
			name: "main",
			status: "connected",
			created_at: "2024-01-01T00:00:00Z",
		...
		vi.mocked(extractAgents).mockReturnValueOnce([singleAgent]);
		...
		const multipleAgents: WorkspaceAgent[] = [
			singleAgent,
			{ ...singleAgent, id: "agent-2", name: "secondary" },
		];
		vi.mocked(extractAgents).mockReturnValueOnce(multipleAgents);
