...
import { activate, handleRemoteAuthority, handleRemoteSetupError, handleUnexpectedAuthResponse } from "./extension"
...
describe("Extension", () => {
  ...
  let mockRemoteSSHExtension: any
  ...
  beforeEach(async () => {
    ...
    vi.mocked(vscode.extensions.getExtension).mockReturnValue(mockRemoteSSHExtension)
    vi.mocked(vscode.workspace.getConfiguration).mockReturnValue({
      get: vi.fn(() => false),
    } as any)

    vi.mocked(Storage).mockImplementation(() => mockStorage as any)
    vi.mocked(Commands).mockImplementation(() => mockCommands as any)
    vi.mocked(WorkspaceProvider).mockImplementation(() => mockWorkspaceProvider as any)
    vi.mocked(Remote).mockImplementation(() => ({}) as any)

    vi.mocked(apiModule.makeCoderSdk).mockResolvedValue(mockRestClient as any)
  ...
  describe("activate", () => {
    it("should throw error when Remote SSH extension is not found", async () => {
      ...
      await expect(activate(mockContext)).rejects.toThrow("Remote SSH extension not found")
    ...
    it("should create and configure tree views for workspaces", async () => {
      ...
      expect(vscode.window.createTreeView).toHaveBeenCalledWith("myWorkspaces", {
        treeDataProvider: mockWorkspaceProvider,
      })
      expect(vscode.window.createTreeView).toHaveBeenCalledWith("allWorkspaces", {
        treeDataProvider: mockWorkspaceProvider,
      })
    ...
    it("should handle authentication failure gracefully", async () => {
      ...
      mockRestClient.getAuthenticatedUser.mockRejectedValue(new Error("401 Unauthorized"))
    ...
    it("should handle autologin when enabled and not logged in", async () => {
      ...
      const mockConfig = {
        get: vi.fn((key: string) => {
          if (key === "coder.autologin") return true
          if (key === "coder.defaultUrl") return "https://auto.coder.example.com"
          return undefined
        }),
      }
    ...
    it("should not trigger autologin when no default URL is configured", async () => {
      ...
      const mockConfig = {
        get: vi.fn((key: string) => {
          if (key === "coder.autologin") return true
          if (key === "coder.defaultUrl") return undefined
          return undefined
        }),
      }
      vi.mocked(vscode.workspace.getConfiguration).mockReturnValue(mockConfig as any)
  ...
  describe("URI handler", () => {
    ...
    beforeEach(async () => {
      ...
      const registerCall = vi.mocked(vscode.window.registerUriHandler).mock.calls[0]
    ...
    it("should handle /open URI with required parameters", async () => {
      const mockUri = {
        path: "/open",
        query: "owner=testuser&workspace=testworkspace&agent=main&folder=/workspace&openRecent=true&url=https://test.coder.com&token=test-token",
      }

      const params = new URLSearchParams(mockUri.query)
      ...
      expect(mockRestClient.setHost).toHaveBeenCalledWith("https://test.coder.com")
    ...
    it("should handle /openDevContainer URI with required parameters", async () => {
      const mockUri = {
        ...
        query: "owner=testuser&workspace=testworkspace&agent=main&devContainerName=mycontainer&devContainerFolder=/container&url=https://test.coder.com",
      }
    ...
    it("should throw error for unknown URI path", async () => {
      ...
      await expect(uriHandler(mockUri)).rejects.toThrow("Unknown path /unknown")
    })
  ...
  describe("Helper Functions", () => {
    describe("handleRemoteAuthority", () => {
      ...
      it("should setup remote and authenticate client when details are returned", async () => {
        ...
        await handleRemoteAuthority(
          mockVscodeWithAuthority as any,
          mockStorage,
          mockCommands,
        ...
        expect(mockRestClient.setHost).toHaveBeenCalledWith("https://remote.coder.example.com")
        expect(mockRestClient.setSessionToken).toHaveBeenCalledWith("remote-token")
      })
    ...
    describe("handleRemoteSetupError", () => {
      ...
      it("should handle CertificateError", async () => {
        const certError = new Error("Certificate error") as any
        ...
        await handleRemoteSetupError(certError, vscode as any, mockStorage, mockRemote)

        expect(mockStorage.writeToCoderOutputChannel).toHaveBeenCalledWith(
          "x509: certificate signed by unknown authority"
        )
        expect(certError.showModal).toHaveBeenCalledWith("Failed to open workspace")
        expect(mockRemote.closeRemote).toHaveBeenCalled()
      })
      ...
      it("should handle AxiosError", async () => {
        ...
        const isAxiosErrorSpy = vi.spyOn(axiosModule, "isAxiosError").mockReturnValue(true)
        const getUriSpy = vi.spyOn(axiosModule.default, "getUri").mockReturnValue("https://api.coder.example.com/workspaces")

        // Mock getErrorMessage and getErrorDetail  
        const errorModule = await import("./error")
        const getErrorDetailSpy = vi.spyOn(errorModule, "getErrorDetail").mockReturnValue("Unauthorized access")
        
        // Import and mock getErrorMessage from the API module
        const coderApiErrors = await import("coder/site/src/api/errors")
        const getErrorMessageSpy = vi.spyOn(coderApiErrors, "getErrorMessage").mockReturnValue("Unauthorized")

        await handleRemoteSetupError(axiosError, vscode as any, mockStorage, mockRemote)

        expect(mockStorage.writeToCoderOutputChannel).toHaveBeenCalledWith(
          expect.stringContaining("API GET to 'https://api.coder.example.com/workspaces' failed")
        )
        expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
      ...
      it("should handle generic errors", async () => {
        ...
        const axiosModule = await import("axios")
        const isAxiosErrorSpy = vi.spyOn(axiosModule, "isAxiosError").mockReturnValue(false)

        await handleRemoteSetupError(genericError, vscode as any, mockStorage, mockRemote)

        expect(mockStorage.writeToCoderOutputChannel).toHaveBeenCalledWith("Generic setup error")
        expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
          "Failed to open workspace",
          expect.objectContaining({
