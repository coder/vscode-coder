...
describe("Inbox", () => {
	...
	let mockSocket: {
		on: vi.MockedFunction<
			(event: string, handler: (...args: unknown[]) => void) => void
		>;
		off: vi.MockedFunction<
			(event: string, handler: (...args: unknown[]) => void) => void
		>;
		close: vi.MockedFunction<() => void>;
		terminate: vi.MockedFunction<() => void>;
		readyState: number;
		OPEN: number;
		CLOSED: number;
	};
	...
	describe("constructor", () => {
		it("should create WebSocket connection with correct URL and headers", () => {
			...
			expect(WebSocket).toHaveBeenCalledWith(expect.any(URL), {
				agent: mockHttpAgent,
			...
			expect(websocketUrl.searchParams.get("templates")).toContain(
