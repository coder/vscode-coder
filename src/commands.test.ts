...
import { OpenableTreeItem as _OpenableTreeItem } from "./workspacesProvider";
		...
		from: vi
			...
			.mockImplementation(
				(options: {
					...
					path: options.path,
					toString: () =>
						`${options.scheme}://${options.authority}${options.path}`,
				}),
			),
	},
	env: {
...
vi.mock("./util", () => ({
	toRemoteAuthority: vi.fn(
		(baseUrl: string, owner: string, name: string, agent?: string) => {
			const host = baseUrl.replace("https://", "").replace("http://", "");
			return `coder-${host}-${owner}-${name}${agent ? `-${agent}` : ""}`;
		},
	),
	toSafeHost: vi.fn((url: string) =>
...
interface MockQuickPick {
	...
	onDidChangeValue: ReturnType<typeof vi.fn>;
...
describe("Commands", () => {
	...
	describe("basic Commands functionality", () => {
		...
		beforeEach(() => {
			...
			vi.mocked(mockRestClient.getAuthenticatedUser).mockResolvedValue(
				mockUser,
			);
		...
		it("should view logs when path is set", async () => {
			...
			vi.mocked(vscode.Uri.file).mockReturnValue(mockUri as vscode.Uri);
	...
	describe("workspace operations", () => {
		...
		it("should open workspace from sidebar", async () => {
			await commands.openFromSidebar(mockTreeItem as _OpenableTreeItem);
		...
		it("should use first recent workspace when openRecent=true with multiple workspaces", async () => {
			...
			await commands.openFromSidebar(treeItemWithoutPath as _OpenableTreeItem);
		...
		it("should open new window when no folder path available", async () => {
			...
			expect(vscode.commands.executeCommand).toHaveBeenCalledWith(
				...
				{
					remoteAuthority:
						"coder-coder.example.com-testuser-testworkspace-main",
					reuseWindow: true,
				},
			);
		});
	...
	describe("maybeAskAgent", () => {
		...
		it("should throw error when no matching agents", async () => {
			...
			await expect(
				commands.maybeAskAgent(mockWorkspace, "nonexistent"),
			).rejects.toThrow("Workspace has no matching agents");
		});
		...
		it("should create correct items for multiple agents", async () => {
			...
			mockQuickPick.onDidChangeSelection.mockImplementation(() => ({
				dispose: vi.fn(),
			}));
	...
	describe("URL handling methods", () => {
			...
			vi.mocked(vscode.workspace.getConfiguration).mockReturnValue({
				get: vi.fn((key: string) => {
					if (key === "coder.defaultUrl") {
						return "https://default.coder.com";
					}
					return undefined;
				}),
			} as vscode.WorkspaceConfiguration);
		...
		describe("askURL", () => {
			it("should show URL picker with default and recent URLs", async () => {
				...
				mockQuickPick.onDidChangeSelection.mockImplementation((callback) => {
					setTimeout(
						() => callback([{ label: "https://selected.coder.com" }]),
						0,
					);
					return { dispose: vi.fn() };
				});
				...
				mockQuickPick.onDidChangeValue.mockImplementation(() => ({
					dispose: vi.fn(),
				}));

				const result = await (
					commands as Commands & { askURL: () => Promise<string | undefined> }
				).askURL();

				expect(mockQuickPick.value).toBe("https://default.coder.com");
				expect(mockQuickPick.placeholder).toBe("https://example.coder.com");
				expect(mockQuickPick.title).toBe(
					"Enter the URL of your Coder deployment.",
				);
			...
			it("should use provided selection as initial value", async () => {
				...
				mockQuickPick.onDidChangeSelection.mockImplementation((callback) => {
					setTimeout(
						() => callback([{ label: "https://provided.coder.com" }]),
						0,
					);
					return { dispose: vi.fn() };
				});
				...
				const result = await (
					commands as Commands & { askURL: () => Promise<string | undefined> }
				).askURL("https://provided.coder.com");
			...
			it("should update items when value changes", async () => {
				vi.mocked(vscode.window.createQuickPick).mockReturnValue(mockQuickPick);
				let valueChangeCallback: (value: string) => void;
				let selectionCallback: (items: readonly vscode.QuickPickItem[]) => void;
		...
		describe("maybeAskUrl", () => {
				...
				const _askURLSpy = vi
					.spyOn(
						commands as Commands & {
							askURL: () => Promise<string | undefined>;
						},
						"askURL",
					)
					.mockResolvedValue("https://asked.coder.com");

				const result = await commands.maybeAskUrl(null);

				expect(_askURLSpy).toHaveBeenCalled();
			...
			it("should normalize URL by removing trailing slashes", async () => {
				const result = await commands.maybeAskUrl(
			...
			it("should use lastUsedUrl as selection when asking", async () => {
				const _askURLSpy = vi
					...
					.mockResolvedValue("https://result.coder.com");

				await commands.maybeAskUrl(null, "https://last.coder.com");

				expect(_askURLSpy).toHaveBeenCalledWith("https://last.coder.com");
			});
		});
	});
	...
	describe("maybeAskToken", () => {
		...
		it("should return user and blank token for non-token auth", async () => {
			...
			const result = await (
				commands as Commands & { askURL: () => Promise<string | undefined> }
			).maybeAskToken("https://coder.example.com", "", false);
		...
		it("should handle certificate error in non-token auth", async () => {
			...
			vi.mocked(mockRestClient.getAuthenticatedUser).mockRejectedValue(
				certError,
			);
		...
		it("should write to output channel for autologin errors", async () => {
			...
			const result = await (
				commands as Commands & { askURL: () => Promise<string | undefined> }
			).maybeAskToken("https://coder.example.com", "", true);
				...
				async (options?: vscode.InputBoxOptions) => {
					if (options.validateInput) {
						...
						const validationResult =
							await options.validateInput("invalid-token");
	...
	describe("openAppStatus", () => {
		beforeEach(() => {
			vi.mocked(mockStorage.getUrl).mockReturnValue(
				"https://coder.example.com",
			);
			vi.mocked(mockStorage.fetchBinary).mockResolvedValue("/path/to/coder");
			vi.mocked(mockStorage.getSessionTokenPath).mockReturnValue(
				"/session/token",
			);
			vi.mocked(vscode.window.createTerminal).mockReturnValue(mockTerminal);
	...
	describe("workspace selection in open method", () => {
		...
		it("should show workspace picker when no arguments provided", async () => {
			...
			const maybeAskAgentSpy = vi
				.spyOn(commands, "maybeAskAgent")
				.mockResolvedValue({
					name: "main",
					expanded_directory: "/workspace",
				} as import("coder/site/src/api/typesGenerated").WorkspaceAgent);
	...
	describe("updateWorkspace", () => {
		...
		it("should update workspace when user confirms", async () => {
			...
			expect(mockRestClient.updateWorkspaceVersion).toHaveBeenCalledWith(
				workspace,
			);
		});
	...
	describe("navigation methods", () => {
		...
		it("should navigate to workspace from tree item", async () => {
			await commands.navigateToWorkspace(mockTreeItem as _OpenableTreeItem);
		...
		it("should navigate to current workspace when no tree item", async () => {
			...
			await commands.navigateToWorkspace(null as _OpenableTreeItem | null);
	...
	describe("error handling", () => {
		it("should throw error if not logged in for openFromSidebar", async () => {
			...
			await expect(
				commands.openFromSidebar(mockTreeItem as _OpenableTreeItem),
		...
		it("should call open() method when no tree item provided to openFromSidebar", async () => {
			...
			await commands.openFromSidebar(null as _OpenableTreeItem | null);
