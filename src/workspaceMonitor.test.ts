...
describe("WorkspaceMonitor", () => {
	...
	let mockEventSource: {
		addEventListener: vi.MockedFunction<
			(event: string, handler: (event: MessageEvent) => void) => void
		>;
	...
	let mockStatusBarItem: {
		text: string;
		tooltip: string;
		show: vi.MockedFunction<() => void>;
		hide: vi.MockedFunction<() => void>;
		dispose: vi.MockedFunction<() => void>;
	};
	...
	describe("notification logic", () => {
		...
		it("should notify about outdated workspace and handle update action", async () => {
			...
			vi.mocked(mockRestClient.getTemplateVersion).mockResolvedValue(
				mockVersion,
			);
			vi.mocked(vscode.window.showInformationMessage).mockResolvedValue(
				"Update",
