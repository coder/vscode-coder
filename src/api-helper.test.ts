...
import { errToStr, extractAllAgents, extractAgents, AgentMetadataEventSchema, AgentMetadataEventSchemaArray } from "./api-helper"
import { Workspace, WorkspaceAgent, WorkspaceResource } from "coder/site/src/api/typesGenerated"
...
describe("errToStr", () => {
  ...
  it("should return API error response message when isApiErrorResponse returns true", () => {
    ...
    expect(errToStr(apiErrorResponse, defaultMessage)).toBe("API response error")
  })
  ...
  it("should handle ErrorEvent with code and message", () => {
    ...
    Object.defineProperty(errorEvent, "code", { value: "E001", writable: true })
    Object.defineProperty(errorEvent, "message", { value: "Connection failed", writable: true })

    vi.mocked(isApiError).mockReturnValue(false)
    vi.mocked(isApiErrorResponse).mockReturnValue(false)

    expect(errToStr(errorEvent, defaultMessage)).toBe("E001: Connection failed")
  })

  it("should handle ErrorEvent with code but no message", () => {
    const errorEvent = new ErrorEvent("error")
    Object.defineProperty(errorEvent, "code", { value: "E002", writable: true })
    Object.defineProperty(errorEvent, "message", { value: "", writable: true })

    vi.mocked(isApiError).mockReturnValue(false)
    vi.mocked(isApiErrorResponse).mockReturnValue(false)

    expect(errToStr(errorEvent, defaultMessage)).toBe("E002: Default error message")
  })

  it("should handle ErrorEvent with message but no code", () => {
    const errorEvent = new ErrorEvent("error")
    Object.defineProperty(errorEvent, "code", { value: "", writable: true })
    Object.defineProperty(errorEvent, "message", { value: "Network timeout", writable: true })
  ...
  it("should return string error when error is non-empty string", () => {
    ...
    expect(errToStr("String error message", defaultMessage)).toBe("String error message")
  })
