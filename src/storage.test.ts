...
describe("Storage", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(fs.mkdir).mockImplementation(() => Promise.resolve(undefined));
		...
		describe("getRemoteSSHLogPath", () => {
			it("should return path to Remote SSH log file", async () => {
				...
				expect(fs.readdir).toHaveBeenCalledWith(
					"/logs/output_logging_20240102",
				);
			});

			it("should return undefined when no output logging directories found", async () => {
				vi.mocked(fs.readdir).mockResolvedValueOnce(["other-dir"] as string[]);
			...
			it("should return undefined when no Remote SSH log file found", async () => {
				vi.mocked(fs.readdir)
					.mockResolvedValueOnce(["output_logging_20240101"] as string[])
					.mockResolvedValueOnce([
						"extension1.log",
						"extension2.log",
					] as string[]);
			...
			it("should use latest output logging directory", async () => {
				vi.mocked(fs.readdir)
					...
					.mockResolvedValueOnce(["Remote - SSH.log"] as string[]);
	...
	describe("CLI configuration", () => {
		describe("configureCli", () => {
			it("should update both URL and token", async () => {
				const updateUrlSpy = vi
					.spyOn(
						storage as Storage & {
							updateUrlForCli: (label: string, url: string) => Promise<void>;
						},
						"updateUrlForCli",
					)
		...
		describe("updateUrlForCli", () => {
			it("should write URL to file when URL provided", async () => {
				const updateUrlForCli = (
					storage as Storage & {
						updateUrlForCli: (url: string) => Promise<void>;
					}
				).updateUrlForCli.bind(storage);

				await updateUrlForCli("test-label", "https://test.com");

				expect(fs.mkdir).toHaveBeenCalledWith("/global/storage/test-label", {
					recursive: true,
				});
				expect(fs.writeFile).toHaveBeenCalledWith(
					"/global/storage/test-label/url",
					"https://test.com",
				);
			});
		...
		describe("updateTokenForCli", () => {
			it("should write token to file when token provided", async () => {
				const updateTokenForCli = (
					storage as Storage & {
						updateTokenForCli: (label: string, token: string) => Promise<void>;
					}
				).updateTokenForCli.bind(storage);
	...
	describe("fetchBinary", () => {
		...
		let mockWriteStream: NodeJS.WritableStream;
		...
		beforeEach(() => {
			...
			vi.mocked(createWriteStream).mockReturnValue(
				mockWriteStream as NodeJS.WritableStream,
			);
		...
		it("should download new binary when version does not match", async () => {
			...
			vi.mocked(vscode.window.withProgress).mockImplementation(
				async (options, callback) => {
					...
					setTimeout(() => {
						const closeHandler = mockReadStream.on.mock.calls.find(
							(call) => call[0] === "close",
						)?.[1];
						if (closeHandler) {
							closeHandler();
			...
			expect(result).toBe("/global/storage/test-label/bin/coder");
			expect(fs.mkdir).toHaveBeenCalledWith("/global/storage/test-label/bin", {
				recursive: true,
			});
		});
		...
		it("should throw error when downloads are disabled", async () => {
			...
			await expect(
				storage.fetchBinary(mockRestClient, "test-label"),
			).rejects.toThrow(
		...
		it("should handle download cancellation", async () => {
			...
			vi.mocked(vscode.window.withProgress).mockImplementation(async () => {
				const _progress = { report: vi.fn() };
				const _token = { onCancellationRequested: vi.fn() };
			...
			await expect(
				storage.fetchBinary(mockRestClient, "test-label"),
			).rejects.toThrow("User aborted download");
		});
