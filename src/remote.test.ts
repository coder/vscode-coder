...
class TestableRemote extends Remote {
  ...
  public waitForAgentConnection(agent: any, monitor: any) {
    return super.waitForAgentConnection(agent, monitor)
  }

  public handleWorkspaceBuildStatus(restClient: any, workspace: any, workspaceName: string, globalConfigDir: string, binPath: string, attempts: number, writeEmitter: any, terminal: any) {
    return super.handleWorkspaceBuildStatus(restClient, workspace, workspaceName, globalConfigDir, binPath, attempts, writeEmitter, terminal)
  }

  public initWriteEmitterAndTerminal(writeEmitter: any, terminal: any) {
    return super.initWriteEmitterAndTerminal(writeEmitter, terminal)
  }

  public createNetworkRefreshFunction(networkInfoFile: string, updateStatus: any, isDisposed: any) {
    return super.createNetworkRefreshFunction(networkInfoFile, updateStatus, isDisposed)
  }

  public handleSSHProcessFound(disposables: any[], logDir: string, pid: number | undefined) {
    return super.handleSSHProcessFound(disposables, logDir, pid)
  }

  public handleExtensionChange(disposables: any[], remoteAuthority: string, workspace: any, agent: any) {
    return super.handleExtensionChange(disposables, remoteAuthority, workspace, agent)
  }

  // Expose private methods for testing
  public testGetLogDir(featureSet: any) {
    return (this as any).getLogDir(featureSet)
  }

  public testFormatLogArg(logDir: string) {
    return (this as any).formatLogArg(logDir)
  }

  public testUpdateSSHConfig(restClient: any, label: string, hostName: string, binaryPath: string, logDir: string, featureSet: any) {
    return (this as any).updateSSHConfig(restClient, label, hostName, binaryPath, logDir, featureSet)
  }

  public testFindSSHProcessID(timeout?: number) {
    return (this as any).findSSHProcessID(timeout)
  }

  public testShowNetworkUpdates(sshPid: number) {
    return (this as any).showNetworkUpdates(sshPid)
  }

  public testMaybeWaitForRunning(restClient: any, workspace: any, label: string, binPath: string) {
    return (this as any).maybeWaitForRunning(restClient, workspace, label, binPath)
  }

  public testConfirmStart(workspaceName: string) {
    return (this as any).confirmStart(workspaceName)
  }

  public testRegisterLabelFormatter(remoteAuthority: string, owner: string, workspace: string, agent?: string) {
    return (this as any).registerLabelFormatter(remoteAuthority, owner, workspace, agent)
  }
}
...
describe("Remote", () => {
  ...
  describe("waitForAgentConnection", () => {
    ...
    it("should wait for agent to connect", async () => {
      ...
      mockMonitor.onChange.event.mockImplementation((callback: any) => {
  ...
  describe("initWriteEmitterAndTerminal", () => {
    ...
    it("should use existing emitter and terminal when provided", () => {
      ...
      const result = remote.initWriteEmitterAndTerminal(mockEmitter, mockTerminal)
  ...
  describe("handleWorkspaceBuildStatus", () => {
    it("should handle pending workspace status", async () => {
      ...
      const updatedWorkspace = { ...workspace, latest_build: { status: "running" } }
    ...
    it("should handle stopped workspace with user confirmation", async () => {
      ...
      const confirmStartSpy = vi.spyOn(remote as any, "confirmStart").mockResolvedValue(true)
      
      const { startWorkspaceIfStoppedOrFailed } = await import("./api")
      const startedWorkspace = { ...workspace, latest_build: { status: "running" } }
      vi.mocked(startWorkspaceIfStoppedOrFailed).mockResolvedValue(startedWorkspace)
    ...
    it("should return undefined when user declines to start stopped workspace", async () => {
      ...
      const confirmStartSpy = vi.spyOn(remote as any, "confirmStart").mockResolvedValue(false)
  ...
  describe("handleSSHProcessFound", () => {
    ...
    it("should setup network monitoring when PID exists", async () => {
      ...
      const showNetworkUpdatesSpy = vi.spyOn(remote as any, "showNetworkUpdates").mockReturnValue(mockDisposable)
      
      const fs = await import("fs/promises")
      vi.mocked(fs.readdir).mockResolvedValue(["123.log", "456-123.log", "other.log"])
  ...
  describe("handleExtensionChange", () => {
    it("should register label formatter", () => {
      ...
      const mockDisposable = { dispose: vi.fn() }
      const registerLabelFormatterSpy = vi.spyOn(remote as any, "registerLabelFormatter").mockReturnValue(mockDisposable)

      remote.handleExtensionChange(disposables, "remote-authority", workspace, agent)
  ...
  describe("findSSHProcessID", () => {
    it("should find SSH process ID successfully", async () => {
      mockStorage.getRemoteSSHLogPath = vi.fn().mockResolvedValue("/path/to/ssh.log")
      const searchSSHLogForPIDSpy = vi.spyOn(remote, "searchSSHLogForPID").mockResolvedValue(12345)
    ...
    it("should timeout when no process found", async () => {
      mockStorage.getRemoteSSHLogPath = vi.fn().mockResolvedValue("/path/to/ssh.log")
      const searchSSHLogForPIDSpy = vi.spyOn(remote, "searchSSHLogForPID").mockResolvedValue(undefined)
  ...
  describe("confirmStart", () => {
    it("should return true when user confirms start", async () => {
      mockVscodeProposed.window.showInformationMessage.mockResolvedValue("Start")
  ...
  describe("showNetworkUpdates", () => {
    it("should create status bar item and periodic refresh", () => {
      ...
      const createNetworkRefreshFunctionSpy = vi.spyOn(remote, "createNetworkRefreshFunction").mockReturnValue(() => {})
  ...
  describe("maybeWaitForRunning", () => {
    it("should return running workspace immediately", async () => {
      ...
      mockVscodeProposed.window.withProgress = vi.fn().mockImplementation(async (options, callback) => {
        return await callback()
      })

      const result = await remote.testMaybeWaitForRunning(mockRestClient, workspace, "test-label", "/bin/coder")
    ...
    it("should handle workspace build process", async () => {
      ...
      mockStorage.getSessionTokenPath = vi.fn().mockReturnValue("/session/token")
      const handleWorkspaceBuildStatusSpy = vi.spyOn(remote, "handleWorkspaceBuildStatus")
        .mockResolvedValue({ workspace: runningWorkspace, writeEmitter: undefined, terminal: undefined })

      mockVscodeProposed.window.withProgress = vi.fn().mockImplementation(async (options, callback) => {
        return await callback()
      })

      const result = await remote.testMaybeWaitForRunning(mockRestClient, initialWorkspace, "test-label", "/bin/coder")
  ...
  describe("registerLabelFormatter", () => {
    it("should register label formatter with agent", () => {
      const mockDisposable = { dispose: vi.fn() }
      mockVscodeProposed.workspace.registerResourceLabelFormatter.mockReturnValue(mockDisposable)

      const result = remote.testRegisterLabelFormatter("remote-authority", "owner", "workspace", "agent")

      expect(mockVscodeProposed.workspace.registerResourceLabelFormatter).toHaveBeenCalledWith({
    ...
    it("should register label formatter without agent", () => {
      ...
      const result = remote.testRegisterLabelFormatter("remote-authority", "owner", "workspace")
  ...
  describe("updateSSHConfig", () => {
    ...
    beforeEach(async () => {
      ...
      const mockGet = vi.fn().mockImplementation((key) => {
        if (key === "remote.SSH.configFile") return null
        if (key === "sshConfig") return []
        return null
      })
      const mockGetConfiguration = vi.fn().mockImplementation((section) => {
        if (section === "coder") return { get: vi.fn().mockReturnValue([]) }
        return { get: mockGet }
      })
      vi.mocked(vscode.workspace).getConfiguration = mockGetConfiguration
    })
    ...
    it("should update SSH config successfully", async () => {
      ...
      vi.mocked(mergeSSHConfigValues).mockReturnValue({ StrictHostKeyChecking: "no" })
      ...
      vi.spyOn(remote as any, "formatLogArg").mockResolvedValue(" --log-dir /logs")
    ...
    it("should handle 404 error from deployment config", async () => {
      ...
      mockRestClient.getDeploymentSSHConfig = vi.fn().mockRejectedValue(axiosError)
      ...
      vi.spyOn(remote as any, "formatLogArg").mockResolvedValue("")
    ...
    it("should handle SSH config property mismatch", async () => {
      ...
      vi.spyOn(remote as any, "formatLogArg").mockResolvedValue("")
      const closeRemoteSpy = vi.spyOn(remote, "closeRemote").mockResolvedValue()
      mockVscodeProposed.window.showErrorMessage.mockResolvedValue("Reload Window")
      const reloadWindowSpy = vi.spyOn(remote, "reloadWindow").mockResolvedValue()
