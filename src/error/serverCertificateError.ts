...
export class ServerCertificateError extends CertificateError {
	...
	private constructor(
		...
		this.name = "ServerCertificateError";
	}
	...
	// maybeWrap returns a ServerCertificateError if the code is a certificate error
	// otherwise it returns the original error.
	static async maybeWrap<T>(
		err: T,
		address: string,
		logger: Logger,
	): Promise<ServerCertificateError | T> {
		if (isAxiosError(err)) {
			switch (err.code) {
				case X509_ERR_CODE.UNABLE_TO_VERIFY_LEAF_SIGNATURE:
					...
					try {
						const cause =
							await ServerCertificateError.determineVerifyErrorCause(address);
						return new ServerCertificateError(err.message, cause);
					} catch (error) {
						logger.warn(`Failed to parse certificate from ${address}`, error);
						break;
					}
				case X509_ERR_CODE.DEPTH_ZERO_SELF_SIGNED_CERT:
					return new ServerCertificateError(
						err.message,
	...
	private allowInsecure(): void {
		...
		vscode.window.showInformationMessage(
			ServerCertificateError.InsecureMessage,
		);
	}
	...
	async showNotification(
		...
		const val = await vscode.window.showErrorMessage(
			title || this.x509Err,
			{ modal, useCustom: modal, detail: this.x509Err },
			...
			// ServerCertificateError.ActionAllowInsecure,
		);
		switch (val) {
			case ServerCertificateError.ActionAllowInsecure:
				this.allowInsecure();
				break;
			case undefined:
				break;
